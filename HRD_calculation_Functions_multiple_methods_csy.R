#####Input data: seg
## the structure of the segmenation matrix. Data was generated from EaCoN package  (https://github.com/gustaveroussy/EaCoN) with the pair-CEL files of Oncoscan and OncoScanCNV data
## The data was generate from R code: Process_pair-cel_files_OncoScan_and_OncoScanCNV.R
...
calc.lst <-function(
  ...
  nA=7,
  chr.arm='no',
  return.loc = FALSE # Return location of LST
){
  nB <- nA+1
  samples <- unique(seg[,1])
  output <- matrix(ncol=1, nrow=length(samples))
  ...
  for(j in samples){
    ...
    for(i in chroms){
      ...
      if(chr.arm == 'no'){
        ...
        LST <- rep(NA, nrow(p.arm)) # Add a column to call LST
        p.arm <- cbind(p.arm, LST)
        LST <- rep(NA, nrow(q.arm)) # Add a column to call LST
        q.arm <- cbind(q.arm, LST)
        if(nrow(p.arm) > 0){
      ...
      p.arm %<>% select(SampleName, Chr, Start, End, TCN, nMajor, nMinor, LST)
      q.arm %<>% select(SampleName, Chr, Start, End, TCN, nMajor, nMinor, LST)
      pq.arm = rbind(p.arm, q.arm)
      
      if(i==chroms[1]) {
        all.arm=pq.arm
      }else{
        all.arm = rbind(all.arm, pq.arm)
      }
    }
    output[j, 1] <- sum(sample.lst)
    if(j==samples[1]) {
      seg.lst = all.arm
    }else{
      seg.lst = rbind(seg.lst, all.arm)
    }
...
#########################################################################
# This function calculates LOH HRD
